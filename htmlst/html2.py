...
# elements which cannot contain children (and text) and denote the end of a sentence
EMPTY_ELEMENTS = {'area', 'base', 'br', 'embed', 'hr', 'img'}

# elements which denote the end of a sentence and whose text content never qualifies as a sentence
...
class HTMLSentenceTokenizer:
    ...
    def tag_should_change_ignored_parent_count(self, tag_name):
        return tag_name in SENTENCE_VOID_ELEMENTS or (self.ignore_header_text and tag_name in HEADER_ELEMENTS)
    ...
    def handle_starttag(self, tag_name):
        # if this tag is the child of an SVE or it is a header element or user would like to ignore headers
        if self.ignored_parent_count > 0:
            if self.tag_should_change_ignored_parent_count(tag_name):
        ...
        if self.raise_invalid_tags:
            raise InvalidTagError("Parsing a tag which is not in the accepted element types. It is of type "
                                  "{}".format(tag_name))
        else:
            self.ignored_parent_count += 1
    ...
    def handle_endtag(self, tag_name):
        """
        Note that unlike handle_starttag and handle_empty_tag, this method does not raise an InvalidTagError because
        html5lib will make all end tags have a corresponding start tag. So, the parser will have already raised an error
        if an invalid tag was parser (and if self.raise_invalid_tags).
        :param tag_name:
        :return:
        """
        if self.tag_should_change_ignored_parent_count(tag_name):
        ...
        if tag_name in BLOCK_LEVEL_ELEMENTS:
            if self.ignored_parent_count < 1:
                self.handle_end_of_string()
            return

        # if this tag is an invalid element.
